...
public class PostgreSQLStorage {
    ...
    public PostgreSQLStorage(SXBans plugin) {
        this.plugin = plugin;
    ...
    public void savePunishment(Punishment punishment) {
        String sql = "INSERT INTO punishments (id, player_uuid, player_name, ip_address, type, reason, " +
                ...
                "reason = EXCLUDED.reason, duration = EXCLUDED.duration, end_time = EXCLUDED.end_time, " +
                "status = EXCLUDED.status, remover_uuid = EXCLUDED.remover_uuid, " +
                "remover_name = EXCLUDED.remover_name, removed_time = EXCLUDED.removed_time, " +
                "remove_reason = EXCLUDED.remove_reason, updated_at = EXCLUDED.updated_at, " +
                "server_name = EXCLUDED.server_name, world_name = EXCLUDED.world_name, " +
                "ip_country = EXCLUDED.ip_country, ip_city = EXCLUDED.ip_city, " +
                "client_version = EXCLUDED.client_version, client_brand = EXCLUDED.client_brand";
    ...
    public List<Punishment> getActivePunishments(UUID playerUUID) {
        List<Punishment> punishments = new ArrayList<>();
        String sql = "SELECT * FROM punishments WHERE player_uuid = ? AND status = 'ACTIVE' AND " +
                "(duration <= 0 OR end_time > ?) ORDER BY start_time DESC";
    ...
    public List<Punishment> getPunishmentsByIP(String ip) {
        List<Punishment> punishments = new ArrayList<>();
        String sql = "SELECT * FROM punishments WHERE ip_address = ? ORDER BY start_time DESC";
        ...
        try (Connection conn = dbManager.getConnection();
            ...
            plugin.getLogger().severe("Failed to get punishments by IP: " + e.getMessage());
        }
        return punishments;
    }
    ...
    public List<HistoryEntry> getAllHistory() {
        ...
        try (Connection conn = dbManager.getConnection();
            ...
            plugin.getLogger().severe("Failed to get all history: " + e.getMessage());
        }
        return history;
    }
    ...
    public void saveIPData(IPData ipData) {
        String sql = "INSERT INTO ip_data (ip_address, country, city, isp, hostname, first_seen, " +
                ...
                "total_logins = ip_data.total_logins + 1, " +
                "total_punishments = EXCLUDED.total_punishments, is_blacklisted = EXCLUDED.is_blacklisted";
    ...
    public IPData getIPData(String ip) {
        ...
        try (Connection conn = dbManager.getConnection();
            ...
            if (rs.next()) {
                return extractIPData(rs);
            }

        } catch (SQLException e) {
            plugin.getLogger().severe("Failed to get IP data: " + e.getMessage());
        }
        return null;
    }
    ...
    public List<IPData> getAllIPData() {
        ...
        String sql = "SELECT * FROM ip_data ORDER BY last_seen DESC";
        ...
        try (Connection conn = dbManager.getConnection();
            ...
            while (rs.next()) {
                ipDataList.add(extractIPData(rs));
            }

        } catch (SQLException e) {
            plugin.getLogger().severe("Failed to get all IP data: " + e.getMessage());
        }
        return ipDataList;
    }
    ...
    private IPData extractIPData(ResultSet rs) throws SQLException {
